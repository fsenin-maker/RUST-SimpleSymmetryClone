...
{
    [Info("SimpleSymmetryClone", "GrokAI", "1.3.3")]
    ...
    public class SimpleSymmetryClone : RustPlugin
    {
        ...
        // Prefab ID для решёток
        private const ulong WindowBarsMetalPrefabId = 1429861300;
        private const ulong WindowBarsWoodPrefabId = 1379457182;

        // Допустимые типы: n = поворот, m = отражение
        private static readonly string[] ValidSymmetryTypes = { "n2s", "n3s", "n4s", "n6s", "m2s", "m4s" };

        // Кэшированные LayerMask'и
        private readonly int _constructionLayer = LayerMask.GetMask("Construction");
        private readonly int _deployableLayer = LayerMask.GetMask("Deployable");

        #endregion

        #region Классы данных

        private readonly Dictionary<ulong, SymmetryData> _playerData = new();
        ...
        private void CmdSym(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                if (data.UiEnabled) CreateUI(player);
            ...
            switch (sub)
            {
                ...
                case "set":
                    if (SetCenter(player))
                    {
                        if (args.Length > 1 && args[1].ToLower() == "auto") AutoDetectType(player);
                        UpdateUI(player);
                    }
                    break;
        ...
        private void CmdUI(BasePlayer player, string cmd, string[] args)
        {
            ...
            switch (args[0])
            {
                ...
                case "SetBtn":
                    if (SetCenter(player)) AutoDetectType(player);
                    UpdateUI(player);
                    break;
        ...
        private bool SetCenter(BasePlayer player)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(player.eyes.HeadRay(), out hit, 100f, _constructionLayer))
            {
        ...
        private void AutoDetectType(BasePlayer player)
        {
            ...
            List<BuildingBlock> foundations = new();
            Vis.Entities(center, DetectRadius, foundations, _constructionLayer);
            ...
            {
                data.Type = "n4s";
                SendReply(player, "Мало фундаментов → n4s по умолчанию.");
                return;
            }
            ...
            {
                data.Type = "n4s";
                SendReply(player, "Недостаточно углов → n4s по умолчанию.");
                return;
            }
        ...
        private void CreateUI(BasePlayer player)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = "-220 20", OffsetMax = "220 200" }
            }, "Hud", UILayer);

            // Заголовок
            container.Add(new CuiLabel
            {
                Text = { Text = "СИММЕТРИЧНОЕ СТРОИТЕЛЬСТВО", FontSize = 18, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
            ...
            container.Add(new CuiLabel
            {
                Text = { Text = data.Enabled ? "<color=#00ff00>АКТИВНО</color>" : "<color=#ff0000>НЕАКТИВНО</color>", FontSize = 18, Align = TextAnchor.MiddleCenter },
            ...
            container.Add(new CuiLabel
            {
                Text = { Text = centerText, FontSize = 14, Align = TextAnchor.MiddleCenter },
            ...
            container.Add(new CuiLabel
            {
                Text = { Text = $"Тип: <color=#ffd700>{data.Type.ToUpper()}</color>", FontSize = 15, Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = "0 0.45", AnchorMax = "1 0.55" }
            }, UILayer, "TypeLabel");

            // Кнопки
            AddButton(container, "ToggleBtn", data.Enabled ? "ВЫКЛ." : "ВКЛ.", "0.05 0.28", "0.45 0.42", data.Enabled ? "0.7 0.3 0.3 0.9" : "0.3 0.7 0.3 0.9");
            AddButton(container, "SetBtn", "УСТ. ЦЕНТР", "0.5 0.28", "0.95 0.42", "0.3 0.5 0.8 0.9");
            AddButton(container, "DeleteBtn", "УДАЛИТЬ ЦЕНТР", "0.05 0.15", "0.95 0.27", "0.7 0.3 0.3 0.9");
            ...
            for (int i = 0; i < ValidSymmetryTypes.Length; i++)
            {
                ...
                AddButton(container, "Type" + type, type.ToUpper(), $"{xMin} 0.02", $"{xMax} 0.13", color);
            }
        ...
        private void OnEntityBuilt(Planner planner, GameObject pool)
        {
            var player = planner?.GetOwnerPlayer();
            var entity = pool?.GetComponent<BaseEntity>();
            if (player == null || entity == null || !pool.IsBuilding) return;
            if (!permission.UserHasPermission(player.UserIDString, PermUse)) return;

            var data = GetData(player.userID);
            if (!data.Enabled || !data.Center.HasValue || !data.CenterRot.HasValue) return;
            if (!IsBuildableEntity(entity)) return;
            ...
            if (isMirrored)
            {
                var normal1 = (centerRot * Vector3.forward).normalized;
                symmetries.Add(new List<Vector3> { normal1 });
                if (sides >= 4)
                {
                    var normal2 = (centerRot * Vector3.right).normalized;
            ...
            timer.Once(0.05f, () =>
            {
                ...
                foreach (var sym in symmetries)
                {
                    ...
                    if (isMirrored)
                    {
                        ...
                        if (entity is Door) newLocalRot *= Quaternion.Euler(0, 180, 0); // Только для копии
                    }
                    else
                    {
                    ...
                    {
                        if (!buildCost.All(c => player.inventory.GetAmount(c.itemid) >= (int)c.amount))
                        {
        ...
        private void OnBuildingBlockUpgrade(BuildingBlock block, BasePlayer player, BuildingGrade.Enum grade)
        {
            ...
            timer.Once(0.05f, () =>
            {
                foreach (var sym in symmetries)
                {
                    ...
                    {
                        Vector3 vec = newLocalPos;
                        foreach (var n in sym) vec = Vector3.Reflect(vec, n);
                        newLocalPos = vec;
                    }
                    else
                    {
                    ...
                    try
                    {
                        Vis.Entities(newPos, 0.5f, entities, _constructionLayer);
                        var symBlock = entities
                            .FirstOrDefault(e => e is BuildingBlock b && b.OwnerID == player.userID &&
        ...
        private List<ItemAmount> GetDeployCost(string prefab)
        {
            ...
            return prefab.Contains("door.hinged.wood") ? CreateItemList(("wood", 300)) :
                   prefab.Contains("door.hinged.metal") ? CreateItemList(("metal.fragments", 150)) :
                   prefab.Contains("door.hinged.toptier") ? CreateItemList(("metal.refined", 20), ("gears", 5)) :
                   prefab.Contains("door.double.hinged.wood") ? CreateItemList(("wood", 350)) :
                   prefab.Contains("door.double.hinged.metal") ? CreateItemList(("metal.fragments", 200)) :
                   prefab.Contains("door.double.hinged.toptier") ? CreateItemList(("metal.refined", 25), ("gears", 5)) :
                   prefab.Contains("wall.frame.garagedoor") ? CreateItemList(("metal.fragments", 300), ("gears", 2)) :
                   prefab.Contains("window.bars.wood") ? CreateItemList(("wood", 50)) :
                   prefab.Contains("window.bars.metal") ? CreateItemList(("metal.fragments", 25)) :
                   null;
        }
        ...
        private List<ItemAmount> CreateItemList(params (string itemName, int amount)[] items)
        {
            ...
            foreach (var (itemName, amount) in items)
            {
                ...
                if (itemDef != null)
                {
                    result.Add(new ItemAmount(itemDef, amount));
                }
            }
...
namespace Oxide.Plugins
{
    ...
    public class SimpleSymmetryClone : RustPlugin
    {
        ...
        // Допустимые типы: n = поворот (normal), m = отражение (mirror)
        private static readonly string[] ValidSymmetryTypes = { "n2s", "n3s", "n4s", "n6s", "m2s", "m4s" };
        ...
        private bool SetCenter(BasePlayer player)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(player.eyes.HeadRay(), out hit, 100f, Rust.LayerMask.Construction))
            {
                data.Center = hit.point;
                data.CenterRot = player.transform.rotation;
                SendReply(player, "Центр симметрии установлен.");
            ...
            timer.Once(0.1f, () =>
            {
                ...
                foreach (var sym in symmetries)
                {
                    ...
                    if (isMirrored)
                    {
                        ...
                        newLocalPos = vec;
                        newLocalRot = rotMatrix.rotation;
                        if (entity is Door) newLocalRot *= Quaternion.Euler(0, 180, 0);
                    }
                    else
                    {
                        var symRot = Quaternion.Euler(sym[0]);
                    ...
                    if (buildCost != null && buildCost.Count > 0)
                    {
                        bool canAfford = buildCost.All(c => player.inventory.GetAmount(c.itemid) >= (int)c.amount);
                        if (!canAfford)
                        {
                            SendReply(player, "Недостаточно ресурсов для симметричной копии.");
                            continue;
                ...
                foreach (var sym in symmetries)
                {
                    ...
                    try
                    {
                        ...
                        var symBlock = entities.FirstOrDefault(e =>
                            e is BuildingBlock b && b.OwnerID == player.userID &&
                            Vector3.Distance(e.transform.position, newPos) < 0.1f) as BuildingBlock;

                        if (symBlock == null || symBlock.grade == grade) continue;
                        symBlock.SetGrade(grade);
                        symBlock.health = symBlock.MaxHealth();
                        symBlock.SendNetworkUpdateImmediate();
                    }
